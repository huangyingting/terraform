...
resource "azurerm_virtual_network_peering" "hub1-to-spoke1" {
  ...
  depends_on                   = [azurerm_virtual_network.hub1, azurerm_virtual_network.spoke1]
}
...
resource "azurerm_virtual_network_peering" "hub1-to-spoke2" {
  ...
  allow_gateway_transit        = false
  use_remote_gateways          = false
  depends_on                   = [azurerm_virtual_network.hub1, azurerm_virtual_network.spoke2]
}
...
resource "azurerm_virtual_network_peering" "hub2-to-spoke3" {
  ...
  allow_gateway_transit        = false
  use_remote_gateways          = false
  depends_on                   = [azurerm_virtual_network.hub2, azurerm_virtual_network.spoke3]
}
...
resource "azurerm_virtual_network_peering" "hub1-to-hub2" {
  name                         = "${azurerm_virtual_network.hub1.name}-To-${azurerm_virtual_network.hub2.name}"
  resource_group_name          = azurerm_resource_group.hubhubnva.name
  virtual_network_name         = azurerm_virtual_network.hub1.name
  remote_virtual_network_id    = azurerm_virtual_network.hub2.id
  allow_virtual_network_access = true
  ...
  allow_gateway_transit        = false
  use_remote_gateways          = false
  depends_on                   = [azurerm_virtual_network.hub1, azurerm_virtual_network.hub2]
}

resource "azurerm_virtual_network_peering" "hub2-to-hub1" {
  name                         = "${azurerm_virtual_network.hub2.name}-To-${azurerm_virtual_network.hub1.name}"
  resource_group_name          = azurerm_resource_group.hubhubnva.name
  virtual_network_name         = azurerm_virtual_network.hub2.name
  remote_virtual_network_id    = azurerm_virtual_network.hub1.id
  allow_virtual_network_access = true
